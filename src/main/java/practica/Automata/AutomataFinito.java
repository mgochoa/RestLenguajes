...
public class AutomataFinito {
    ...
    public void EliminarExtranos() {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < col; j++) {
                ...
                row[j] = automata[i][j];
                if (i == 1 & j == 0) {
                    if (nuevoStack(estadosViejos, row[j])) {
                        estadosViejos.add(row[j]);
                    }

                }
                if (i == 1 & j > 1) {
                    if (nuevoStack(estadosNuevos, row[j]) & nuevoStack(estadosViejos, row[j])) {
   ...
   public String[][] CrearAutomataSimplificado() {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < col; j++) {
                if (i == 0) {
                    ...
                    } else if (nuevoStack(estadosNuevos, automata[i][j]) & nuevoStack(estadosViejos, automata[i][j])) {
                        row[j] = automata[i][j];
                        estadosNuevos.add(row[j]);
                    }
    ...
    public boolean probarHilera(String hilera) {
        ...
        String estadoActual = automata[1][0];
        for (String h : hilera.split("")) {
            int i = encontrarSimbolo(h);
            estadoActual = EncontrarTransicion(estadoActual, i);
            respuesta = encontrarAceptacion(estadoActual);

        }
        return respuesta;
    }

    public int encontrarSimbolo(String simb) {
        for (int i = 2; i < automata[0].length; i++) {
            if (automata[0][i].equals(simb)) {
                return i;
            }
        }
        return -1;
    ...
    public boolean encontrarAceptacion(String es) {
        for (int i = 1; i < automata.length; i++) {
            if (automata[i][0].equals(es)) {
